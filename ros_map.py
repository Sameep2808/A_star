...
def DS(node, goal, L,ga):
	...
	while Q:
		...
		print(cn.d[0],cn.d[1])
		cv2.waitKey(1)
		if (((cn.d[1] - goal[1]) ** 2) + ((cn.d[0] - goal[0]) ** 2) <= ((2*L) ** 2)):
			return cn,CL,OLc,P
		for a in action_set:
			NewNode = move(a,cn,L) 
			if NewNode != None:
...
def get_input():
	...
	print('Enter Goal X (Range: 0 - 1000):')
	xg = int(input())
	if (xg - rr <0) or (xg + rr >xmax):
		print('INVALID X SETTING GOAL X AS 1000')
		xg=xmax - rr
		
	print('Enter Goal Y (Range: 0 - ymax):')
	yg = int(input())
	if (yg - rr <0) or (yg + rr>ymax):
		print('INVALID Y SETTING GOAL Y AS 1000')
		yg=ymax - rr 
	
	if detect(xg,yg):
		print('INVALID POINTS SETTING INITIAL POINT AS [1000,1000]')
		xg=xmax - rr
		yg=ymax - rr
	ga = 0
	print('Enter RPM1: (5 - 10)')
	rpm1 = int(input())
	print('Enter RPM2: (5 - 10)')
	rpm2 = int(input())
...
def graph(O,OP,P):
	...
	for c,p in zip(O,OP):
		x,y = c
		px,py = p
		ax.quiver(px,py,x-px,y-py,units='xy' ,scale=1, width = 0.5,headwiReadMedth = 10,headlength = 10)
	for z in P:
		x,y = z.d
		px,py = z.pxy
...
def Vig(O,OP,P):
	fourcc = cv2.VideoWriter_fourcc(*'mp4v')
	out = cv2.VideoWriter("Output.mp4", fourcc, 10, (1000,1000))
